...
const PhongbanPhanquyenWorkHours = ({ onSubmit, activeTab, initialData }) => {
  ...
  const [phanQuyenDotCong, setPhanQuyenDotCong] = useState([]); //Lấy theo id đợt
  // Hàm lấy dữ liệu phòng ban
  const fetchDepartments = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/phanquyen/phongban/phan-quyen-dot-cong`
  ...
  useEffect(() => {
    const validDepartments = sortedDepartments
      .map((department) => {
        ...
        const phanQuyenTime = phanQuyenDotCong.find(
    ...
    // console.log("validDepartments", validDepartments);
    onSubmit(validDepartments);
  }, [departmentTimes, fixedTime, sortedDepartments, phanQuyenDotCong]);

  const getDateTimeValue = (date) => {
    return date instanceof Date && !isNaN(date) ? date : null;
  };

  const handlePhanQuyenWorkHours = async () => {
    const response = await axios.get(
      `${API_BASE_URL}/phanquyen/phongban/phan-quyen-dot-cong/${initialData}`
    );
    setPhanQuyenDotCong(response.data);
  };
  ...
  useEffect(() => {
    if (phanQuyenDotCong.length > 0) {
      const initialTimes = {};
      phanQuyenDotCong.forEach((item) => {
  ...
  }, [phanQuyenDotCong, initialData]);

  return (
    <>
      <h4>Chọn phòng ban xem công</h4>
      <div className="row">
...
export default PhongbanPhanquyenWorkHours;

...
exports.getSalarysByQLOLD = async (req, res) => {
  try {
...
exports.getSalarysByQL = async (req, res) => {
  try {
    const { phutrach } = req.params;
...
exports.getSalarysByTT = async (req, res) => {
  try {
    const { phutrach } = req.params;
    const { page = 1, limit = 50 } = req.query; // Mặc định page 1, limit 50
...
exports.getSalarysTTByIDNV = async (req, res) => {
  try {
...
exports.getSalarysQLByIDNV = async (req, res) => {
  try {
...
exports.getComplaintDetailsCT = async (req, res) => {
  try {
    ...
    let query = `
      SELECT DISTINCT
        ...
        ct.*
      FROM pm_luong_dot d
      INNER JOIN pm_luong_status_luong sl ON d.id = sl.id_dot
      INNER JOIN pm_luong_chi_trong ct ON ct.id = sl.id_trong
      INNER JOIN sync_data_hi_time_sheet.sync_nhan_vien nv ON ct.ma_nv = nv.ma_nv
...
exports.getComplaintDetailsCN = async (req, res) => {
  try {
    const { dotId, maNV, status, phongban } = req.query;
    ...
    let query = `
      ...
      FROM pm_luong_dot d
      INNER JOIN pm_luong_status_luong sl ON d.id = sl.id_dot
      INNER JOIN pm_luong_chi_ngoai cn ON cn.id = sl.id_ngoai
      INNER JOIN sync_data_hi_time_sheet.sync_nhan_vien nv ON cn.ma_nv = nv.ma_nv
			LEFT JOIN sync_data_hi_time_sheet.sync_bo_phan bp ON nv.id_bo_phan = bp.id
      LEFT JOIN pm_phong_ban_quan_ly_new pb ON pb.id = bp.id_phong_ban
      LEFT JOIN sync_data_hi_time_sheet.sync_phong_ban pbnew ON pbnew.id = pb.id_phong_ban
      WHERE 1=1
...
exports.getSalaryStatusOLD = async (req, res) => {
  try {
